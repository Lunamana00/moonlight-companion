...
Clipboard sync uses the same shape of transport: Moonlight Companion keeps separate TCP channels open for Mac-to-Windows and Windows-to-Mac clipboard payloads. Payloads are still encoded as ZIP archives for text, images, and file drops, and the older shared ZIP polling path remains available as a fallback. Mac clipboard helper import/export calls are bounded by `MOONLIGHT_CLIPBOARD_HELPER_TIMEOUT_SECONDS` so a stuck clipboard or file operation can fail, retry, and leave later transfers flowing instead of wedging the sync loop. Windows clipboard TCP receive sockets use `MOONLIGHT_CLIPBOARD_TCP_IO_TIMEOUT_MS` for read/write operations, so half-open Mac-to-Windows sends cannot block later sends.

For direct file transfer from Mac to Windows, drag files or folders from Finder toward the Moonlight window. Companion detects the file drag near Moonlight, including fast pointer paths that cross the Moonlight frame, then briefly latches that hit and magnetically keeps the whole Moonlight window as a drop target for that drag. The drop target accepts modern file URL drag items, newline-separated file URL strings, plain-text file URL or absolute-path lists whose local paths exist and are readable, Finder filename pasteboard entries, and macOS file promises from source apps that create the file only after the drop is accepted. The Companion window also keeps a fallback drop target and a small floating drop strip. While sending, Companion shows the dropped item names, quick file-size summaries when they are cheap to read, and live phases for metadata collection, packaging, TCP/SSH send, and Windows receive confirmation; folder sizes are measured in the background transfer path so large folder drops do not freeze the GUI first. If another transfer, reveal, or transfer test is already running, new file drops are queued and sent after the current operation finishes successfully; cancelling or failing the busy operation clears those queued drops. Files within the clipboard payload limit are sent over the existing Mac-to-Windows clipboard TCP channel, imported into the Windows clipboard as file drops, and copied into the configured Windows receive folder. Finder file clipboard sync uses the same TCP acknowledgement path and records the confirmed Windows receive paths for later reveal actions; the GUI watches that state while it is open, updates the status line when background receives finish, and defers that notice until the current GUI operation is idle when needed. Oversized Finder file clipboards and oversized drops can instead be copied directly into the Windows receive folder over SSH; after the copy, the Windows GUI agent is asked to put the received paths onto the Windows clipboard too, so large files can still be pasted when that handoff succeeds. Transient failures in that background direct path are retried while the Mac clipboard item is unchanged. Current Windows agents acknowledge clipboard imports on the same TCP connection, so Companion can quickly show that the receive-folder import was confirmed, including the actual Windows receive-folder names and paths for later reveal actions; otherwise it falls back to the older SSH state check and clearly marks confirmation as pending if no confirmation arrives. Moonlight window and strip drops wait longer for fallback confirmation when automatic paste or reveal is enabled, then send `Ctrl+V` only after Windows confirms the import, the Windows clipboard is ready, and Moonlight is already focused; if Moonlight is not focused, Companion does not bring it forward by default and leaves the files ready on the Windows clipboard. Enable `Bring Moonlight forward before auto-paste` when you want the old focus-changing paste behavior. The GUI can also ask Windows to reveal the latest confirmed received item again or put it back onto the Windows clipboard if Explorer was hidden, automatic reveal is disabled, or another clipboard action replaced the file drop; if only some remembered Windows receive items are still available, reveal/copy actions use the remaining paths while reporting the missing items, and the remembered latest receive state is trimmed to those remaining paths for follow-up actions.
...
For Windows to Mac transfer, copy files in Windows Explorer. The Windows agent exports that file clipboard payload over the Windows-to-Mac TCP channel when it fits the clipboard payload limit, and current Mac receivers acknowledge the import before the Windows agent treats TCP delivery as complete. The Windows agent accepts longer acknowledgement response lines for file-name summaries while keeping incoming TCP headers tightly bounded, so long received names do not unnecessarily leave the SSH fallback ZIP in place. Oversized file copies can still be exposed as the existing SSH fallback ZIP, and failed or unacknowledged TCP sends keep that fallback available, so the Mac sync agent pulls files into the configured Mac receive folder even when they are too large for the live TCP clipboard channel or TCP confirmation is unavailable. The Mac receiver places received files on the macOS clipboard, records the latest received Mac file paths, and notifies macOS with the received file names and total size so you can paste into Finder. Mac receive-folder imports are staged before final placement, and stale hidden staging folders from interrupted older receives are cleaned up when the next Mac receive-folder import begins. When `MOONLIGHT_TRANSFER_REVEAL_MAC_DIR` is enabled, Finder reveals the newly received files directly instead of only opening the receive folder. If a Windows file clipboard points at a missing or unreadable Windows path, the Windows agent records that stale source and the Mac sync agent mirrors it into a macOS notification and the Companion status line; the open Companion status line clears after the Windows clipboard issue is resolved. If Companion is open, its status line also updates when new Mac files arrive; receives that arrive while another GUI operation is busy are shown after that operation finishes. The GUI can reveal the last received Mac files again if the notification was missed or Finder moved behind other windows, or copy the latest received files back onto the Mac clipboard if another clipboard action replaced them before you pasted. If only some remembered Mac receive items are still available, the GUI keeps the usable actions enabled, reports how many items remain, and trims the remembered latest receive state to those remaining items after reveal or copy so follow-up actions stop retrying missing files. That clipboard restore also refreshes the local sync state so it does not get mirrored back to Windows as a new send.
...
Use `Test File Transfer` in the GUI to refresh the Windows agent, start the Mac transfer services, stream the current test step into the status line, create small temporary and empty files, PNG image files, Korean file names, Windows-safe Mac file and folder name conversion, file names with spaces, apostrophes, and newline-safe state encoding, multi-item selections, nested folders, empty subfolders, the background Finder-style Mac file clipboard path, stale Windows file clipboard failure state, oversized background file clipboards, oversized direct multi-item drops, Windows-to-Mac SSH fallback multi-item pulls, and latest Mac receive clipboard restores, verify both directions, confirm the Mac-to-Windows receive-folder import, check same-name collision handling, and clean up the test payloads without opening Terminal. Supported Mac clipboard contents are restored after the test, including file clipboards that point at original local paths; while the test runs, background Mac-to-Windows clipboard polling is paused except for its dedicated clipboard-path checks, so test clipboard mutations and the final restore are not mirrored as ordinary user clipboard changes.
