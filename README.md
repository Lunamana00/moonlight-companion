...
Windows receive-folder imports, including oversized direct SSH transfers, are staged before final placement so failed or interrupted payloads do not leave partial files in the visible receive folder.
...
All receive-folder imports are collision-safe and staged before final placement; oversized direct Mac-to-Windows copies use a hidden Windows staging folder before the final move.
