...
For direct file transfer from Mac to Windows, drag files or folders from Finder toward the Moonlight window. Companion detects the file drag near Moonlight, including fast pointer paths that cross the Moonlight frame, and temporarily turns the whole Moonlight window into a drop target. The Companion window also keeps a fallback drop target and a small floating drop strip. While sending, Companion shows the dropped item names and total source size. Files are sent over the existing Mac-to-Windows clipboard TCP channel, imported into the Windows clipboard as file drops, and copied into the configured Windows receive folder. When the Windows agent reports back, Companion shows that the receive-folder import was confirmed; otherwise it clearly marks confirmation as pending. Moonlight window and strip drops can send `Ctrl+V` automatically after transfer so the files land in the focused Windows app, then notify macOS with the result.
