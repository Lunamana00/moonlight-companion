...
Use `Test File Transfer` in the GUI to refresh the Windows agent, start the Mac transfer services, stream the current test step into the status line, create small temporary and empty files, PNG image files, Korean file names, Windows-safe Mac file and folder name conversion, file names with spaces, apostrophes, and newline-safe state encoding, multi-item selections, nested folders, empty subfolders, the background Finder-style Mac file clipboard path, oversized direct multi-item drops, Windows-to-Mac SSH fallback multi-item pulls, and latest Mac receive clipboard restores, verify both directions, confirm the Mac-to-Windows receive-folder import, check same-name collision handling, and clean up the test payloads without opening Terminal. Supported Mac clipboard contents are restored after the test, including file clipboards that point at original local paths; while the test runs, background Mac-to-Windows clipboard polling is paused except for its dedicated clipboard-path check, so test clipboard mutations and the final restore are not mirrored as ordinary user clipboard changes.
