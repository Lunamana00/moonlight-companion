...
- The file transfer self-test now verifies partial Windows receive reveals with multiple remaining items, including the machine-readable remaining path output.
...
- Partial Windows-to-Mac receive notices now use singular/plural grammar for the remaining available item count.
...
- Partial `Reveal Last Windows Receive` now selects the remaining received item, or opens the remaining items' containing folder, instead of only opening the top-level receive folder.
...
- `Reveal Last Windows Receive` status text now distinguishes selected items, containing-folder opens, and receive-folder opens for items spread across different folders.
- Automatic Windows receive-result reveal now uses the same explicit selected/containing-folder/different-folder status text as `Reveal Last Windows Receive`.
